      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-2">
            ...
            {/* NEW TAB: Thuốc Thú Y & Dược Phẩm Trị Liệu */}
            <div
              className="relative"
            ...
            >
              <Link
                href="/pharmacy"
                className="px-3 py-2 rounded-xl text-sm font-semibold text-rose-700 hover:text-rose-800 bg-rose-50/70 hover:bg-rose-100/70 transition-colors flex items-center gap-1.5"
              >
              ...
              {pharmacyDropdown && (
                <div className="absolute left-0 mt-1 w-72 rounded-2xl bg-white shadow-2xl border border-rose-100 p-2 space-y-1 z-50 animate-in fade-in zoom-in-95">
                  <Link
                    href="/pharmacy?group=antiparasitic"
                    className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-rose-50 text-xs font-semibold text-slate-700 hover:text-rose-600 transition-colors"
                  >
                    <span className="text-base">🪱</span>
                    <div>
                      <p className="font-bold text-slate-800">Trị Ve Rận & Bọ Chét</p>
                      <p className="text-[10px] text-slate-400 font-normal">Five Axolaner 68mg, 28mg, 11mg</p>
                    </div>
                  </Link>
                  <Link
                    href="/pharmacy?group=dewormer"
                    className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-rose-50 text-xs font-semibold text-slate-700 hover:text-rose-600 transition-colors"
                  >
                    <span className="text-base">💊</span>
                    <div>
                      <p className="font-bold text-slate-800">Tẩy Giun Sán Chó Mèo</p>
                      <p className="text-[10px] text-slate-400 font-normal">Five Alben hương dâu thơm</p>
                    </div>
                  </Link>
                  <Link
                    href="/pharmacy?group=skin_care"
                    className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-rose-50 text-xs font-semibold text-slate-700 hover:text-rose-600 transition-colors"
                  >
                    <span className="text-base">🧴</span>
                    <div>
                      <p className="font-bold text-slate-800">Trị Ghẻ Mạt, Nấm Da & Vệ Sinh</p>
                      <p className="text-[10px] text-slate-400 font-normal">Five Butomec, sữa tắm đặc trị</p>
                    </div>
                  </Link>
                  <div className="pt-1 mt-1 border-t border-slate-100 flex items-center justify-between px-2 py-1">
                    <Link
                      href="/pharmacy#catalog"
                      className="text-[11px] font-bold text-orange-600 hover:underline flex items-center gap-1"
                    >
                      📋 Bảng báo giá & Catalog
                    </Link>
                    <Link
                      href="/pharmacy#calculator"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-100 bg-white px-4 pt-3 pb-6 space-y-2 shadow-xl animate-in slide-in-from-top-2">
          ...
          <Link
            href="/pharmacy"
            onClick={() => setMobileMenuOpen(false)}
            className="block px-4 py-2.5 rounded-xl text-base font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100"
          >
            💊 Thuốc Thú Y & Dược Phẩm Trị Liệu
          </Link>
          <Link
            href="/blog"
            onClick={() => setMobileMenuOpen(false)}
